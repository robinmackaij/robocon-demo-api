...
from robocon_demo_api.storage.database import get_poem, get_poems, store_poem, delete_poem


poem_router = APIRouter(prefix="/poems")


@poem_router.get("", status_code=200, response_model=list[Poem], operation_id="get_all_poems")
...
@poem_router.delete("/{poem_id}", status_code=204, response_class=Response, responses={404: {"model": Detail}})
def delete_poem_(poem_id: UUID, request: Request) -> None:
    api_key = request.headers.get("api-key", "")
    if api_key != CONFIG.api_key:
        raise HTTPException(status_code=401)
    if not get_poem(poem_id=poem_id):
        raise HTTPException(status_code=404, detail=f"Poem with id {poem_id} not found.") from None
